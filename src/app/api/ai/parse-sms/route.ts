...
export async function POST(req: NextRequest) {
  try {
    ...
    const blocked = checkRateLimit(userId, 'ai-parse-sms', RATE_LIMIT_AI);
    ...
    if (regexResult.amount > 0) {
      return NextResponse.json(regexResult, {
        headers: { 'X-Parse-Source': 'regex-low-confidence' },
      });
    }
